//PARTB 5-Knapsack dynamic programming approach
// write this: Time complexity of Naive/bruteforce approach is O(2n) in dynamic it is O(n*W)
...
printf("\n The table of knapsack problem using dynamic programming is:\n");
    for(i=0;i<=n;i++)
     {
      for(j=0;j<=m;j++)
        {
